...
class db_class {
   ...
   function connect($host='', $user='', $pw='', $db='', $persistant=true) {
      ...
      if ($persistant)
         $this->db_link = ($GLOBALS["___mysqli_ston"] = mysqli_connect($this->host,  $this->user,  $this->pw));
      else
         $this->db_link = ($GLOBALS["___mysqli_ston"] = mysqli_connect($this->host,  $this->user,  $this->pw));

      // Check for an error establishing a connection
      if (!$this->db_link) {
         $this->last_error = mysqli_error($GLOBALS["___mysqli_ston"]);
         return false;
      }
   ...
   function select_db($db='') {
      ...
      if (!mysqli_select_db($GLOBALS["___mysqli_ston"], $this->db)) {
         $this->last_error = mysqli_error($GLOBALS["___mysqli_ston"]);
         return false;
      }
   ...
   function get_row($result, $type='MYSQL_BOTH') {
      ...
      if ($type == 'MYSQL_ASSOC') $row = mysqli_fetch_array($result,  MYSQLI_ASSOC);
      if ($type == 'MYSQL_NUM') $row = mysqli_fetch_array($result,  MYSQLI_NUM);
      if ($type == 'MYSQL_BOTH') $row = mysqli_fetch_array($result,  MYSQLI_BOTH);
