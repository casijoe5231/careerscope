...
<div id="tabs">
  <ul>
    ...
    <li><a href="#tabs-3">Subject Score</a></li>
  </ul>
  <div id="tabs-1">
    <?php
...
<div class="row">
    ...
    <div class="col-sm-2">
        ...
        <a class="btn btn-default btn-block" href="selsubject.php" class="button bright">Back</a>
...
<script  type="text/javascript">
	...
	$is=0;  // Technical score
    $it=0;  
	$ss=0;  // Technical score
    $st=0;  
    ...
    while($row=mysqli_fetch_array($res))
    {
        if($row['domain']=="dsp")  // Quantitative
        {
         $qs = $qs + $row['score'];
         $qt = $qt + $row['max_score'];
        }
        else if($row['domain']=="ai")  // Verbal
        {
         $vs = $vs + $row['score'];
         $vt = $vt + $row['max_score'];     
        }
        else if($row['domain']=="css")  // Logical
        {
         $ls = $ls + $row['score'];
         $lt = $lt + $row['max_score'];     
        }
		 else if($row['domain']=="im")  // Logical
        {
         $is = $is + $row['score'];
         $it = $it + $row['max_score'];     
        }
		 else if($row['domain']=="sc")  // Logical
        {
         $ss = $ss + $row['score'];
         $st = $st + $row['max_score'];     
        }
 ...
 if($ss!=0)
    ...
    echo "var line2 = [['Digital Signal Processing',".(int)$qp."], ['Artificial intelligences', ".(int)$vp."], ['Cryptography ans System Security', ".(int)$lp."], ['Image processing', ".(int)$ip."],['Soft Computing', ".(int)$ip."],['ERP And SCM', ".(int)$ip."]];";
    }   
    ...
    ?>
        ...
        var plot2 = $.jqplot('chart2', [line2], {
          height: 400,
          width: 750,
		  title: "Subject Sectional Score",
          
		   seriesColors: ['#ff5645', 'orange', '#ac58fa',  '#33fc34','#dd336d','#BEF781'],
