...
$sql20="select * from goals_completed where self_eval='Yes' and mentor_eval='Yes' and graph='No' and goal_type NOT IN('Specific','Measurable','Ambitious','Realistic','Timebased')";
$res20=mysqli_query($GLOBALS["___mysqli_ston"], $sql20);
while($row20=mysqli_fetch_array($res20))
{

	$sql21="select * from goal_other  where goal_id='$row20[goal_id]'";
	$res21=mysqli_query($GLOBALS["___mysqli_ston"], $sql21);
	$data21=mysqli_fetch_array($res21);
	
	$sql22="select * from goalm_other  where goal_id='$row20[goal_id]'";
...
<!--Body Starts here-->
<?php  
$sql00="select * from graph where stud_email='$email' and goal_type NOT IN('Specific','Measurable','Ambitious','Realistic','Timebased')";
