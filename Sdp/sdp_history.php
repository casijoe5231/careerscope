...
<!--Body Begins here-->
<!--<div class="container">-->
<div class="panel panel-primary col-lg-14">
  <!-- Default panel contents -->
  <div class="panel-heading"><center><h4><?php $user_name = $_SESSION['user'][1]; echo "$user_name";echo "'s Goal History" ?></h4></center></div>
  ...
  <table class="table">
    ...
    <tr>
      ...
      <th>DueDate</th>
	   ...
	   <th>Comp Status</th>
	   <th>Date Compl</th>
	   <th>Percent Goal Comp</th>
	   <th>No Weekly Update</th>
	   <th>Last Date Weekly Update</th>
	   <th>No Times Edited</th>
	   <th>Revive Status</th>
	   <th>Date Revived</th>
...
$sql1="select * from goal_hist where email='$email'";
...
$count=mysqli_num_rows($res1);
	if($count)
	{
		...
		{
			echo '<tr><th scope="row">'.$row1['goal_id'].'</th><td>'.$row1['title'].'</td><td>'.$row1['deadline'].'</td><td>'.$row1['time_added'].'</td><td>'.$row1['time_edited']. '</td><td>'.$row1['time_deleted'].'</td><td>'.$row1['completion_stat'].'</td><td>'.$row1['time_completed'].'</td><td>'.$row1['goal_com'].'</td><td>'.$row1['no_weekly_updates'].'</td><td>'.$row1['last_weekly_date'].'</td><td>'.$row1['no_time_update'].'</td><td>'.$row1['revive_stat'].'</td><td>'.$row1['date_revive'].'</td><td>'.$row1['evidence_stat'].'</td><td>'.$row1['evidence_date'].'</td><td>'.$row1['eval_stat'].'</td><td>'.$row1['rated_by'].'</td><td>'.$row1['eval_score'].'</td></tr>';
		}
	}
	else
	{
