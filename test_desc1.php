...
<a href="student_report1.php?studnt=<?php echo $_SESSION['studnt'] ?>&test=<?php echo $_GET['id'] ?>" style="text-decoration:none;border:1px solid #09F; background:#09F; padding:5px; color:#FFF;  border-radius:5px; width:auto; height:50px;margin-right:10px;">Back</a>
</div>
<br>
<br>
...
$sql2="select * from test_score_detail where test_score_id='$_GET[test_score_id]'";
		...
		{
		echo "<a href='quest_desc1.php?quest_id=".$row2['q_id']."&score_id=".$row2['test_score_id']."&given_ans=".$row2['given_ans']."' style='text-decoration:none'>Question</a>".$quest++;
			if($row2['given_ans']==1)
			{
				echo ": A";
