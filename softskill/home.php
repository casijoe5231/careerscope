...
<!-- Graph plot  -->
   ...
   <script class="include" type="text/javascript" src="jqplot/plugins/jqplot.donutRenderer.min.js"></script>
...
<div id="tabs">
  <ul>
    ...
    <li><a href="#tabs-3">Softskill Test</a></li>
  </ul>
  <div id="tabs-1">
    <?php
...
<script  type="text/javascript">
    ...
    while($row=mysqli_fetch_array($res))
    {
        if($row['domain']=="leader")  // Quantitative
        {
         $qs = $qs + $row['score'];
         $qt = $qt + $row['max_score'];
        }
        else if($row['domain']=="time")  // Verbal
        {
         $vs = $vs + $row['score'];
         $vt = $vt + $row['max_score'];     
        }
        else if($row['domain']=="stress")  // Logical
        {
         $ls = $ls + $row['score'];
         $lt = $lt + $row['max_score'];     
        }
		else if($row['domain']=="comm")  // Logical
        {
         $cs = $cs + $row['score'];
         $ct = $ct + $row['max_score'];     
        }else if($row['domain']=="change")  // Logical
        {
         $c1s = $c1s + $row['score'];
         $c1t = $c1t + $row['max_score'];     
        }
    ...
    echo "var line2 = [['Leadership',".(int)$qp."], ['Time Management', ".(int)$vp."], ['Stress Management ', ".(int)$lp."], ['Communication skill', ".(int)$cp."], ['Change Management', ".(int)$c1p."], ['image Management', ".(int)$tp."]];";
    }   
    ...
    ?>
        ...
        var plot2 = $.jqplot('chart2', [line2], {
          height: 400,
          width: 750,
		  title: "Test Score in each Section of Softskill",
          
		              seriesColors: ['red', 'orange', 'blue',  '#33fc34','#dd336d','indigo'],
          series:[{renderer:$.jqplot.PieRenderer,rendererOptions: {startAngle: 45, fill: false,lineWidth: 5,
 sliceMargin: 3, showDataLabels: true , dataLabels: 'value' }  }],
          legend:{show:true},
        });
